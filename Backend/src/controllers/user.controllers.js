...
export const sendScheduleMeet = asyncHandler(async (req, res) => {
  ...
  displayTime = `${formattedHour}:${minute} ${ampm} (PST)`;

  const to = user.email;
  const subject = "Request for Scheduling a meeting";
  const message = `
<div style="font-family: 'Segoe UI', sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: auto; border: 1px solid #e0e0e0; border-radius: 10px; overflow: hidden;">
  <div style="background-color: #3f3f3f; padding: 20px; text-align: center;">
    <h2 style="color: #ffffff; margin: 0;">Meeting Request Received</h2>
  </div>
  <div style="padding: 30px;">
    <p>Hello,</p>
    <p>You have received a new meeting request from <strong>${req.user.name}</strong>.</p>
    <div style="background-color: #f9f9f9; padding: 15px; border-left: 4px solid #3bb4a1; margin: 20px 0;">
      <p><strong>Proposed Date:</strong> ${date}</p>
      <p><strong>Proposed Time:</strong> ${displayTime}</p>
    </div>
    <p>Please log in to your account at <strong>SwapSkills</strong> to review the request.</p>
  </div>
</div>
`;;
