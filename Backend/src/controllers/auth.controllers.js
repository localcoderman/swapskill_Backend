...
export const AuthHandle = asyncHandler(async (req, res) => {
  ...
  if (existingUser) {
    ...
    res.cookie("accessToken", jwtToken, { httpOnly: true, secure: true, sameSite: "none", path: "/" });
  ...
  const jwtToken = generateJWTToken_email(unregisteredUser);
  res.cookie("accessTokenRegistration", jwtToken, { httpOnly: true, secure: true, sameSite: "none", path: "/" });
